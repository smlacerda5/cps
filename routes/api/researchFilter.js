...
exports = module.exports = function (req, res) {
  ...
  var paginationQuery =  {
    ...
    perPage: 12, // be sure ./views/filterName has same value
    maxPages: 10,
    filters: {
      state: 'published',
  ...
  if (req.query.id) {
    // load posts by location
      ...
      var p = keystone.list('Research').paginate(paginationQuery1)
    ...
    // load all posts
 		view.on('init', function(next) {
      var p = keystone.list('Research').paginate(paginationQuery)
      .sort('-publishedDate')
      .populate('author categories location')
