...
exports = module.exports = function (req, res) {
  ...
  // get the post
	view.on('init', function (next) {
		var p = keystone.list('Event').model.findOne({state: 'published', slug: locals.filters.event})
    .populate('tags')
  ...
  // get next post
  view.on('init', function(next) {
    var p = keystone.list('Event').model
  ...
  // get related posts
  view.on('init', function(next) {
    var q = keystone.list('Event').model.find({state: 'published', locale: locals.filters.locale})
