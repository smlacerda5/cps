...
exports = module.exports = function (req, res) {
  ...
  var paginationQuery =  {
    ...
    perPage: 6, // be sure ./views/filterName has same value
    maxPages: 10,
    filters: {
      state: 'published',
  ...
  if (req.query.id) {
    // load posts by location
      ...
      var p = keystone.list('Press').paginate(paginationQuery1)
    ...
    // load all posts
 		view.on('init', function(next) {
      var p = keystone.list('Press').paginate(paginationQuery)
      .sort('-publishedDate')
      .populate('author categories location')
